...
const createInvoice = asyncHandler(async (req, res) => {
  const lineItemSchema = z.object({
    description: z
      .string()
      .trim()
      .min(1, { message: "Line item description cannot be empty" }),
    quantity: z
      .number()
  ...
  const createInvoiceSchema = z.object({
    ...
    lineItems: z
      ...
      .min(1, { message: "Invoice must have at least one line item" }),
  ...
  const { clientId, issueDate, dueDate, lineItems, tax, notes, terms, status } =
  ...
  if (!organization) {
    throw new ApiError(
      404,
      "Cannot create invoice no organization found for this user"
    );
  }
  ...
  if (!client) {
    throw new ApiError(
      404,
      "Client not found or you do not have the permission to update it"
    );
  }
...
const getAllInvoice = asyncHandler(async (req, res) => {
  ...
  if (!organization) {
    throw new ApiError(
      404,
      "Cannot fetch invoices organization not found for this user"
    );
  }
  ...
  if (clientName?.trim()) {
    ...
    // if no clientName filter, but you still want to show client name
    pipeline.push({
      $lookup: {
        from: "clients",
        localField: "clientId",
...
const getSingleInvoice = asyncHandler(async (req, res) => {
  ...
  if (!organization) {
    throw new ApiError(
      404,
      "Cannot fetch invoice no organization found for this user"
    );
  }
  ...
  if (!invoice) {
    throw new ApiError(
      404,
      "Invoice not found or you do not have the permission to view it"
    );
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {...invoice.toObject(), ...client.toObject()}, "Invoice fetched successfully"));
});
...
const updateInvoice = asyncHandler(async (req, res) => {
  ...
  if (!organization) {
    throw new ApiError(
      404,
      "Cannot update invoice no organization found for this user"
    );
  }
  ...
  if (needsRecalculation) {
    const lineItemsForCalc =
      validationData.lineItems ?? originalInvoice.lineItems;

    const taxRatePercentage =
      validationData.tax?.rate ?? originalInvoice.tax.rate * 100;
